...
def structure_constants(basisA, basisB, operation_mode='commutator', return_extended_basis=False, return_data_tuple=False, tol=1e-12):
...
def _commutant_matrix_opstrings(basis, operator, operation_mode, return_extended_basis):
    ...
    if return_extended_basis:
        return (commutant_matrix, extended_basis)
    else:
        return commutant_matrix

def _commutant_matrix_operators(operators, operator, operation_mode, return_extended_basis):
...
def commutant_matrix(basis, operator, operation_mode='commutator', return_extended_basis=False):
    ...
    return_extended_basis : bool, optional
        ...
        commuting the :math:`\\hat{h}_a` with :math:`\\hat{\\mathcal{O}}`.
        Defaults to False.
    ...
    if isinstance(basis, Basis):
        return _commutant_matrix_opstrings(basis, operator, operation_mode, return_extended_basis)
    elif isinstance(basis, list) and isinstance(basis[0], Operator):
        return _commutant_matrix_operators(basis, operator, operation_mode, return_extended_basis)
    else:
